...
class EnvelopeOwnershipMigrationTest(unittest.TestCase):
    ...
    def test_removes_legacy_morph_without_moving_existing_nodes(self):
        source = preset(volume_active=True, scratch_active=False)
        ...
        graph = {
            ...
            "edges": [
                signal_edge("volume", "env", "volumeMultiply", "right"),
                signal_edge("legacyEnvelopeMorph", "value", "volume", "red"),
                signal_edge("legacyEnvelopeMorph", "value", "volume", "blue"),
            ],
        }
        original_positions = {
            node["id"]: copy.deepcopy(node["position"])
            for node in graph["nodes"]
            if node["id"] != "legacyEnvelopeMorph"
        }

        changed = migration.migrate_pair(source, graph, {})

        self.assertTrue(changed)
        for node_id, position in original_positions.items():
            self.assertEqual(migration.node_with_id(graph, node_id)["position"], position)
        self.assertIsNone(migration.node_with_id(graph, "legacyEnvelopeMorph"))

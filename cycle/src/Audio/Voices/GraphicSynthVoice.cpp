...
void GraphicSynthVoice::calcCycle(VoiceParameterGroup& group)
{
	...
	for(int i = 0; i < samplesThisCycle; ++i)
	{
		...
		sampIndex &= (colSize - 1);
//		int nextSampIndex = int(nextRealSampIndex) & (colSize - 1);
	...
	ippsMulC_32f_I(2.f, layerAccumBuffer[Left], samplesThisCycle);
	ippsCopy_32f(layerAccumBuffer[Left], layerAccumBuffer[Right], samplesThisCycle);

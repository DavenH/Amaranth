	...
	,	scratchGroup			(LayerGroups::GroupScratch)
{
	...
	DeformerPanel& deformer = getObj(DeformerPanel);

	for(int e = 0; e < numElementsInArray(envGroups); ++e)
	{
		EnvRastGroup& group = *envGroups[e];
		group.envGroup.push_back(EnvRenderContext(EnvRasterizer(repo, &deformer, "EnvRasterizer" + String(e) + "_0"), 0));
	...
	for(int i = 0; i < 2; ++i)
		chanMemory[i].resize(getConstant(MaxBufferSize));
								 ...
								 const int currentPitchWheelPosition)
{
	...
	for(int i = 0; i < envRasterizers.size(); ++i)
	{
		envRasterizers[i]->ensureParamSize(audioSource->unisonVoicesAction.getValue());
		envRasterizers[i]->setNoteOn();
	}
...
void SynthesizerVoice::initialiseEnvMeshes()
{
	...
	for(int e = 0; e < numElementsInArray(envGroups); ++e)
	{
		EnvRastGroup& rastGroup = *envGroups[e];
	...
	for(int i = 0; i < envRasterizers.size(); ++i)
	{
		envRasterizers[i]->setNoiseSeed(random.nextInt());
		envRasterizers[i]->updateOffsetSeeds();
	}
}
...
int SynthesizerVoice::getRenderOffset()
{
	int renderOffset =
		...
		currentVoice->getResamplingAlgo() == Resampling::Sinc ?
				Resampler::historyToWindow(getConstant(ResamplerLatency), 1 / currentVoice->getLastRatio()) : 0;
...
void SynthesizerVoice::calcEnvelopeBuffers(int numSamples)
{
	...
	for(int i = 0; i < volumeGroup.size(); ++i)
	{
		...
		{
			bool stillActive = envRast.renderToBuffer(numSamples, deltaX, EnvRasterizer::headUnisonIndex, *props, 1.f); // TODO
...
void SynthesizerVoice::fetchEnvelopeMeshes()
{
		...
		if(numLocalEnvelopes != group.size())
		{
			...
			for(int layerIndex = 0; layerIndex < size; ++layerIndex)
			{
				...
				if(! dynamic_cast<MeshLibrary::EnvProps*>(layer.props)->global)
				{
					...
					group.envGroup.push_back(EnvRenderContext(EnvRasterizer(repo, &deformer, name), layerIndex));
	...
	for(int i = 0; i < envRasterizers.size(); ++i)
	{
		EnvRasterizer& rast = *envRasterizers[i];
...
{
	Ref<Waveform3D> surface		= &getObj(Waveform3D);
	Ref<Spectrum3D> spectrum3D	= &getObj(Spectrum3D);

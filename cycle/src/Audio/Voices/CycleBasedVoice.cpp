	...
	,	timeRasterizer		(repo)
{
	...
	for(int c = 0; c < 2; ++c)
	{
		...
		oversamplers[c] = new Oversampler(repo);
	}
...
CycleBasedVoice::~CycleBasedVoice()
{
}
...
void CycleBasedVoice::initCycleBuffers()
{
	...
	for(int i = 0; i < (int) groups.size(); ++i)
	{
		...
		for(int c = 0; c < 2; ++c)
		{
			...
			group.cycleBuffer[c].reset();
			group.cycleBuffer[c].setMemoryBuffer(buf);

			// pad with one sample to eliminate possibility of resampling problems
			group.cycleBuffer[c].write(0);
...
void CycleBasedVoice::render(StereoBuffer& channelPair)
{
	...
	if(downSample)
	{
		...
		{
			ScopedAlloc<Ipp32f> tempBuffer(numSamples);
	...
	while(frame.frontier < lastSampleToRender)
	{
		...
		for(int i = 0; i < noteState.numUnisonVoices; ++i)
		{
					...
					(singleFrame ? frame.cycleCount < futureFrame.cycleCount : group.cumePos < futureFrame.cumePos))
			{
				...
				float portionOfNext		= singleFrame ? remainder / float(noteState.stride) :
														1 - (futureFrame.cumePos - group.cumePos) / float(noteState.stride * futureFrame.period);
...
//				jassert(portionOfNext <= 1.5f && portionOfNext >= -0.5f);
				...
				int scaledPhase 		= int(noteState.nextPow2 * (unison->getPhase(group.unisonIndex))) & (noteState.nextPow2 - 1);
				...
				for(int c = 0; c < (noteState.isStereo ? 2 : 1); ++c)
				{
					...
					else
					{
						...
						if(doPhaseShift && scaledPhase != 0)
						{
							if(isFirstCycle)
							{
								status(ippsCopy_32f(pastCycle[c], biasedCyc + scaledPhase, noteState.nextPow2 - scaledPhase));
								status(ippsCopy_32f(pastCycle[c] + (noteState.nextPow2 - scaledPhase), biasedCyc, scaledPhase));

								biasedCyc.copyTo(lerpHalf);
							}
							else
							{
								status(ippsCopy_32f(layerAccumBuffer[c], rastBuffer + scaledPhase, noteState.nextPow2 - scaledPhase));
								status(ippsCopy_32f(layerAccumBuffer[c] + (noteState.nextPow2 - scaledPhase), rastBuffer, scaledPhase));

								status(ippsCopy_32f(pastCyc, tempBuffer + scaledPhase, noteState.nextPow2 - scaledPhase));
								status(ippsCopy_32f(pastCyc + (noteState.nextPow2 - scaledPhase), tempBuffer, scaledPhase));
...
void CycleBasedVoice::fillLatency(StereoBuffer& channelPair)
{
	...
	if(cycleCompositeAlgo == Interpolate || parent->flags.haveFilter || parent->flags.haveFFTPhase)
	{
		for(int c = 0; c < 2; ++c)
		{
			Buffer<float> sumBuffer(rastBuffer + c * numSamples, numSamples);
			...
			if(resamplingAlgo == Resampling::Sinc)
			{
				...
				for(int i = 0; i < noteState.numUnisonVoices; ++i)
				{
					...
					ippsAdd_32f_I(resamplerTail, sumBuffer + start,
								  jmin((int) resamplerTail.size(), numSamples - start));
				}
			}
			else
			{
...
inline void CycleBasedVoice::updateEnvelopes(int paramIndex, int deltaSamples)
{
	...
	for(int e = 0; e < numElementsInArray(parent->envGroups); ++e)
	{
		...
		for(int i = 0; i < group.size(); ++i)
		{
			...
			{
				renderRast.rast.renderToBuffer(deltaSamples, noteState.timePerOutputSample, paramIndex, *props, tempoScale);
...
void CycleBasedVoice::updateValue(int outputId, int dim, float value)
{
	...
	if(groupPair.isNotNull())
	{
		...
		EnvRastGroup& group = groupPair.groupId == LayerGroups::GroupVolume ? parent->volumeGroup :
				 	 	   	  groupPair.groupId == LayerGroups::GroupPitch  ? parent->pitchGroup  : parent->scratchGroup;
...
void CycleBasedVoice::unisonVoiceCountChanged()
{
	...
	{
		groups[i].resamplers[Left].freeDlg();
		groups[i].resamplers[Right].freeDlg();
	}
...
float CycleBasedVoice::getScratchTime(int layerIndex, double cumePos)
{
	...
	for(int i = 0; i < (int) parent->scratchGroup.size(); ++i)
	{
		EnvRenderContext& scratch = parent->scratchGroup[i];

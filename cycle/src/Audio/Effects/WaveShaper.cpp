...
Waveshaper::Waveshaper(SingletonRepo* repo) :
		Effect(repo, "Waveshaper")
	,	preamp(1.f)
	,	postamp(1.f)
...
void Waveshaper::init()
{
	...
	for(int i = 0; i < numElementsInArray(oversamplers); ++i)
	{
		oversamplers[i] = new Oversampler(repo, 16);
		oversamplers[i]->setOversampleFactor(1);
	}
...
{
	return (isEnabled() && oversamplers[graphicOvspIndex]->getOversampleFactor() > 1) ? oversamplers[graphicOvspIndex]->getLatencySamples() : 0;
}

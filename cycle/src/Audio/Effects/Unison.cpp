...
Unison::Unison(SingletonRepo* repo) :
		Effect				(repo, "Unison")
	...
	,	setVoicesAction		(SetVoices)
{
	...
	for(int j = 0; j < 2; ++j)
	{
		ParamGroup& group = *groups[j];

		group.voices.push_back(UnivoiceData());
...
void Unison::updatePanning(bool isAudio, bool force)
{
	...
	if(ui->isGroupMode() || force)
	{
		...
		if(order % 2 == 1)
		{
			int i = 0;
			for(; i < order / 2; ++i)
				group.voices[i].pan = 0.5 + group.panScale * 0.5 * cos(IPP_PI * i);
...
{
	return jmin<int>(maxUnisonOrder,
	            int(maxUnisonOrder * value + 1.));
}
...
bool Unison::isStereo()
{
	if(! ui->isGroupMode())
	{
		...
		for(int i = 0; i < (int) audioParams.voices.size(); ++i)
		{
			if(audioParams.voices[i].pan != 0.5f)
				return true;
		}
...
bool Unison::isPhased()
{
	if(! ui->isGroupMode())
	{
		...
		for(int i = 0; i < (int) audioParams.voices.size(); ++i)
		{
			if(audioParams.voices[i].phase != 0.f)
				return true;
		}
...
{
	if(orderValue < 0)
		orderValue = ui->getParamGroup().getKnobValue(Unison::Order);
...
bool Unison::changeOrderTo(bool isAudio, int newOrder)
{
	if(newOrder < 0)
	{
		double orderValue = ui->getParamGroup().getKnobValue(Unison::Order);
...
void Unison::modeChanged()
{
	...
	numVoices = ui->isGroupMode() ?
			...
			jmax<int>((int) graphicParams.voices.size(), calcOrder(ui->getParamGroup().getKnobValue(Unison::Order)));

...
Equalizer::Equalizer(SingletonRepo* repo) :
		Effect(repo, "Equalizer")
	,	samplerate	(44100)
	,	filterOrder	(1)
{
...
void Equalizer::processBuffer(AudioSampleBuffer& buffer)
{
	...
	{
		Buffer<float> destBuffer(buffer.getWritePointer(c), numSamples);
...
{
//	progressMark
	jassert(partitions[0].states[0] != 0);
...
//	progressMark
	...
	for(int c = 0; c < numEqChannels; ++c)
	{
		...
		part.taps[c] 		= Buffer<double>(tapsBuffer + tapsOffset + c * tapsLength, tapsLength);
		part.delayLine[c] 	= Buffer<double>(delayBuffer + delayOffset + c * delayLineLength, delayLineLength);
...
{
	inputBuffer.section((int)inputBuffer.size() - (int) overflowBuffer.size(),
			overflowBuffer.size()).copyTo(overflowBuffer);

	for(int i = 0; i < numPartitions; ++i)
	{
		IppsIIRState64f_32f* state = partitions[i].states[graphicEqChannel];
...
void Equalizer::test()
{
	...
	for(int i = 0; i < numPartitions; ++i)
	{
		...
		dout << "Eq partition " << String(i) << "\n";
		dout << "Gain: " << String(part.gainDB.getCurrentValue(), 2) << "\n";
		dout << "Freq: " << String(part.centreFreq.getCurrentValue(), 2) << "\n";

//		for(int j = 0; j < 2; ++j)
//		{
//			dout << "State: " << part.states[j] << "\n";
//			dout << ""
//		}
...
//	getObj(CsvFile).addValues(fft.getPhases(), 		2);
//	for(int i = 0; i < 2; ++i)
//	{
//		Ipp32f* buffer = audioBuffer.getWritePointer(i);
//
//		ippsZero_32f(buffer, size);
//		ippsSet_32f(1.f, buffer + pulseWidth, pulseWidth);
//		ippsSet_32f(-1.f, buffer + 2 * pulseWidth, pulseWidth);
//	}

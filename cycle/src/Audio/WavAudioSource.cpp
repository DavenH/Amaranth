...
void WavAudioSource::SampleVoice::startNote(int midiNoteNumber,
    ...
    for (int i = 0; i < 2; ++i) {
        resampleState[i].init(stateMem.place(nextPower));
        resampleState[i].reset(ratio);
	}
}
...
WavAudioSource::WavAudioSource(SingletonRepo* repo) :
        SingletonAccessor(repo, "WavAudioSource"), updateOffsetFlag(false) {
    for (int i = 0; i < 6; ++i) {
		SampleVoice* voice = new SampleVoice(repo, this);
		voices.add(voice);
...
bool WavAudioSource::SampleSound::appliesToNote(int midiNoteNumber) {
    return NumberUtils::within(midiNoteNumber,
                               Range<int>(getConstant(LowestMidiNote), getConstant(HighestMidiNote)));
}

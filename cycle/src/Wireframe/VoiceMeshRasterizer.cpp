...
void VoiceMeshRasterizer::generateControlPointsChaining(float oscPhase) {
    if (mesh->getNumCubes() == 0 || state == nullptr) {
	    return;
    }

    if (state->callCount > 0) {
        std::swap(state->backIcpts, controlPoints);
	...
	for(auto cube : mesh->getCubes()) {
        ...
        if (reduct.pointOverlaps) {
            ...
            TrilinearCube::vertexAt(voiceTime, Vertex::Time, a, b, vertex);
    ...
    if (state->backIcpts.size() < 2 || controlPoints.size() < 2) {
        waveX.nullify();
		waveY.nullify();
		state->callCount++;
		unsampleable = true;
    ...
    if (state->callCount > 0) {
		...
		curves.clear();
		curves.reserve(controlPoints.size() + 5 + int(extraPadFront) + int(padFront) + int(padBack) + int(extraPadBack));
		...
		curves.emplace_back(state->frontB, state->frontA, controlPoints[0]);
		curves.emplace_back(state->frontA, controlPoints[0], controlPoints[1]);
		...
		// update front
		{
			end = controlPoints.size() - 1;

			// policy: icpt size is guaranteed to be >= 2 at this point
			state->frontE.assignFront(controlPoints, 5);
			state->frontD.assignFront(controlPoints, 4);
			state->frontC.assignFront(controlPoints, 3);
			state->frontB.assignWithTranslation(controlPoints[end - 1], -1);
			state->frontA.assignWithTranslation(controlPoints[end], 	-1);
		}
...
void VoiceMeshRasterizer::updateCurves() {
	...
	float base = 0.05f / float(CurvePiece::resolution);

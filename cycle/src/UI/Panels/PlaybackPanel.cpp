...
#include "../Audio/GraphicTableAudioSource.h"
...
{
	getObj(WaveformInter2D).performUpdate(UpdateType::Update);
	getObj(WaveformInter2D).locateClosestElement();
	getObj(SpectrumInter2D).performUpdate(UpdateType::Update);
...
void PlaybackPanel::stopAudio()
{
	if (playing)
	{
		getObj(AudioHub).getKeyboardState().noteOff(1, midiKeyPlaying);
	}
}

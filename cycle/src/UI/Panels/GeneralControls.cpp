...
void GeneralControls::paint(Graphics& g) {
	...
	Component& rightTool = editionSplit(nudge, axe);

	mg.drawJustifiedText(g, "TOOL", 		pointer, 	rightTool, 	true, 	toolCO);
	mg.drawJustifiedText(g, "PRESET", 		save, 		next, 		true, 	prstCO);
	mg.drawJustifiedText(g, "TRANSPORT", 	play, 		ffwd, 		true, 	transCO);
	mg.drawJustifiedText(g, "SURF VIEW", 	waveVerts, 	verts, 		true, 	waveCO);
}

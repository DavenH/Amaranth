...
void VertexPropertiesPanel::init()
{
	...
	ampVsPhaseProperties = new VertexProperties(this, String::empty, String::empty, Vertex::Time);

	gainProperties.push_back(ampVsPhaseProperties);

	for(int i = 1; i < (int) properties.size(); ++i)
		gainProperties.push_back(&properties[i]);

	allProperties.push_back(ampVsPhaseProperties);
...
void VertexPropertiesPanel::paint(Graphics & g)
{
	...
	Rectangle<int> tSlider(properties[Vertex::Time].slider->getBounds());
...
void VertexPropertiesPanel::resized()
{
	...
	Rectangle<int> titleBounds = bounds.removeFromTop(10);
	...
	for(int i = 0; i < (int) displayOrder.size(); ++i)
	{
		...
		if(props->id != Vertex::Time)
		{
			...
			props->gain->setBounds(Rectangle<int>(deformBounds.getX(), deformBounds.getY(), size, size));
		}
	  #endif
	...
	sliderArea.toBack();
	sliderArea.setBounds(Rectangle<int>(properties[Vertex::Time].slider->getPosition(),
	...
	deformBounds.removeFromLeft(2);
	int size = jmin(deformBounds.getWidth(), deformBounds.getHeight());
	ampVsPhaseProperties->gain->setBounds(Rectangle<int>(deformBounds.getX(), deformBounds.getY(), size, size));


	gainArea.toBack();
	gainArea.setBounds(Rectangle<int>(properties[Vertex::Red].gain->getPosition(),
	                                  ampVsPhaseProperties->gain->getBounds().getBottomRight()));

	boxArea.toBack();
	boxArea.setBounds(Rectangle<int>(properties[Vertex::Red].dfrmChanBox->getPosition(),
...
void VertexPropertiesPanel::mouseEnter(const MouseEvent& e)
{
	...
	console->updateAll(String::empty, "Parameters of selected vertices. Deformers warp path of parent lines",
...
void VertexPropertiesPanel::sliderValueChanged(Slider* slider)
{
	...
	if(currentInteractor->getCollisionDetector().validate())
	{
		   ...
		   slider == properties[Vertex::Blue].slider)
		{
			...
			MorphPosition pos = getObj(MorphPanel).getMorphPosition();
...
void VertexPropertiesPanel::updateSliderValues(bool ignoreChangeMessage)
{
	...
	if(selected.empty())
	{
		for(int i = 0; i < numSliders; ++i)
		{
			...
			if(i == Vertex::Amp)
			{
				...
				ampVsPhaseProperties->gain->setValue(0.5, ignoreChangeMessage ? dontSendNotification : sendNotificationAsync);
			}
		  #endif
		}
	...
	for(int i = 0; i < (int) properties.size(); ++i)
		properties[i].previousValue = 0;

	for(int i = 0; i < (int) gainProperties.size(); ++i)
		gainProperties[i]->previousGain = 0;
	...
	for(set<VertCube*>::iterator it = lines.begin(); it != lines.end(); ++it)
	{
		VertCube* cube = *it;

		for(int i = 0; i < (int) gainProperties.size(); ++i)
		{
			VertexProperties& props = *gainProperties[i];
	...
	for(int i = 0; i < (int) allProperties.size(); ++i)
	{
		allProperties[i]->setValueToCurrent(! ignoreChangeMessage);
		allProperties[i]->setGainValueToCurrent(! ignoreChangeMessage);
	}
...
void VertexPropertiesPanel::comboBoxChanged(ComboBox* box)
{
	...
	dout << "set guide dim to " << dim << "\n";

	vector<VertCube*> affectedLines;
	vector<int> previousMappings;

	foreach(VertIter, it, selected)
	{
		foreach(VertCube**, cit, (*it)->owners)
		{
			VertCube* cube = *cit;
...
void VertexPropertiesPanel::gainChanged(Slider* slider, int changeType)
{
	...
	for(int i = 0; i < (int) gainProperties.size(); ++i)
	{
		if(gainProperties[i]->gain == slider)
		{
			props = gainProperties[i];
			break;
		}
	}
	...
	if(deformEnabled)
	{
		if(changeType == ValueChanged)
		{
			...
			{
				VertCube* cube = selected[i]->owners.getFirst();
...
Component* VertexPropertiesPanel::getComponent(int which)
{
	switch(which)
	{
		case CycleTour::TargSliderArea: return &sliderArea; 							break;
		case CycleTour::TargTimeSlider: return properties[Vertex::Time].slider; 		break;
		case CycleTour::TargPhsSlider: 	return properties[Vertex::Phase].slider; 		break;
		case CycleTour::TargAmpSlider: 	return properties[Vertex::Amp].slider; 			break;
		case CycleTour::TargKeySlider: 	return properties[Vertex::Red].slider; 			break;
		case CycleTour::TargMorphSlider: return properties[Vertex::Blue].slider; 		break;
		case CycleTour::TargCrvSlider: 	return properties[Vertex::Curve].slider; 		break;

		case CycleTour::TargBoxArea: 	return &boxArea; 								break;
		case CycleTour::TargPhsBox: 	return properties[Vertex::Phase].dfrmChanBox; 	break;
		case CycleTour::TargAmpBox: 	return properties[Vertex::Amp].dfrmChanBox; 	break;
		case CycleTour::TargKeyBox: 	return properties[Vertex::Red].dfrmChanBox; 	break;
		case CycleTour::TargModBox: 	return properties[Vertex::Blue].dfrmChanBox; 	break;
		case CycleTour::TargCrvBox: 	return properties[Vertex::Curve].dfrmChanBox; 	break;
		case CycleTour::TargAvpBox: 	return ampVsPhaseProperties->dfrmChanBox; 		break;

		case CycleTour::TargGainArea: 	return &gainArea; 								break;
		case CycleTour::TargPhsGain: 	return properties[Vertex::Phase].gain; 			break;
		case CycleTour::TargAmpGain: 	return properties[Vertex::Amp].gain; 			break;
		case CycleTour::TargKeyGain: 	return properties[Vertex::Red].gain; 			break;
		case CycleTour::TargModGain: 	return properties[Vertex::Blue].gain; 			break;
		case CycleTour::TargCrvGain: 	return properties[Vertex::Curve].gain; 			break;
		case CycleTour::TargAvpGain: 	return ampVsPhaseProperties->gain; 				break;
	}
...
void VertexPropertiesPanel::refreshValueBoxesFromSelected()
{
	if(currentInteractor != nullptr && currentInteractor->dims.numHidden() > 0)
	{
		...
		for(int i = 0; i < numSliders; ++i)
		{
			...
			if(i == 0)
				props = ampVsPhaseProperties;
			else
				props = &properties[i];

			props->dfrmChanBox->setSelectedId(guideLayerIdx[i] < 0 ? NullDfrmId : guideLayerIdx[i] + 1 + NullDfrmId, dontSendNotification);
		}
	}
}
...
void VertexPropertiesPanel::updateSliderProperties()
{
	...
	for(int i = 0; i < (int) properties.size(); ++i)
	{
		...
		if(props.messager != nullptr)
		{
			props.messager->message =
					itrProps.deformApplicable[i] ? "Set deform channel for " + names[srcId] + " versus " + names[destId] : String::empty;
		}
	...
	if(! itrProps.ampVsPhaseApplicable)
	{
		...
		ampVsPhaseProperties->messager->message = String::empty;
		ampVsPhaseStr = "--";
	}
	else
	{

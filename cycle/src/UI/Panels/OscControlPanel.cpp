...
OscControlPanel::OscControlPanel(SingletonRepo* repo) :
	...
	,	ParameterGroup::Worker(repo, "OscControlPanel")
{
}
...
void OscControlPanel::init()
{
	...
	StringFunction decibel18(StringFunction() .chain(Mul, 2.0).chain(Add,  -1.).chain(Mul, 18.));
	StringFunction octaveStr(StringFunction(0).chain(Add, -0.5).chain(Mul,  4.).chain(Add, 0.5).chain(Rnd));
	StringFunction speedTime(StringFunction(0).chain(Mul,  8.0).chain(Add, -3.).chain(Pow, IPP_E).chain(Rnd));
...
{
	getObj(Updater).update(UpdateSources::SourceOscCtrls, (int) UpdateType::RestoreDetail);
}


void OscControlPanel::reduceDetail()
{
	getObj(Updater).update(UpdateSources::SourceOscCtrls, (int) UpdateType::ReduceDetail);
}

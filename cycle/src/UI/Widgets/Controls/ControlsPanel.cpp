...
void ControlsPanel::paint(Graphics& g) {
	...
	for(int i = 0; i < (int) images.size(); ++i)
		g.drawImageAt(images[i].image, images[i].x, images[i].y);
    ...
    if (isHorz) {
        for (int i = 0; i < (int) bands.size(); ++i) {
            if (bands[i]->isCurrentlyCollapsed() || ! bands[i]->isVisibleDlg())
				continue;

			const Rectangle<int>& bandRect = bands[i]->getBoundsInParentDelegate();
        ...
        for (int i = 0; i < (int) dividedBands.size(); ++i) {
			...
			if(bandCount[dividedBands[i].first] > 1) {
			...
			} else if (bandCount[dividedBands[i].second] > 1) {
...
void ControlsPanel::addRetractableCallout(std::unique_ptr<RetractableCallout>& callout,
	...
	pullout = new PulloutComponent	(getObj(MiscGraphics).getIcon(posX, posY), buttons, repo, ! horz);
	callout = new RetractableCallout(buttons, pullout, horz);

	addAndMakeVisible(callout);
	expandableComponents.add(callout);
}
...
void ControlsPanel::addDividedBand(IDynamicSizeComponent* start, IDynamicSizeComponent* end) {
    ...
    dividedBands.add(std::pair<IDynamicSizeComponent*, IDynamicSizeComponent*>(start, end));
}

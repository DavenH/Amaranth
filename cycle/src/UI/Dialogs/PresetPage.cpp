    ...
    ,	subfolderLabel		({}, "Subfolder: ")
{
    ...
    for(auto & label : labels) {
        ...
        label->setEditable(true, false, true);
...
void PresetPage::refresh() {
    ...
    if (wave.isHighlit()) {
        String exts[] = { "wav", "ogg", "aif", "mp3", "aiff", "flac" };
...
void PresetPage::loadItem(const DocumentDetails& details, bool ignoreSave) {
    ...
    } else if (String(".wav.mp3.ogg.flac.aif.aiff").containsIgnoreCase(file.getFileExtension())) {
...
void PresetPage::paint(Graphics& g) {
    ...
    getObj(MiscGraphics).drawJustifiedText(g, "TAGS", 	Rectangle(0, 0, tagsField.getWidth(), 0), true, &tagsField);
}

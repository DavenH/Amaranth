...
void SpectrumInter2D::init() {
    ...
    setRasterizer(rasterizer, rasterizer, rasterizer, rasterizer, rasterizer);
...
void SpectrumInter2D::showCoordinates() {
    ...
    if (isMagnitudeMode) {
        float absAmp =
                2 * Arithmetic::invLogMapping((float) getConstant(AmpTensionScale) * MathConstants<float>::twoPi, col[currHarmonic]);

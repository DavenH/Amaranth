...
void PluginProcessor::setStateInformation (const void* data, int sizeInBytes)
{
	if(! suspendStateRead)
	{
		...
		if(in.readInt() == getConstant(DocMagicCode))
		{
			...
			std::unique_ptr<XmlElement> xmlState (getXmlFromBinary (xmlBlock.getData(), xmlBlock.getSize()));

			if (xmlState != 0 && xmlState->hasTagName ("PluginSettings"))
				getObj(Settings).readGlobalSettings(xmlState);
			else
			{
				jassertfalse;
			}
...
void PluginProcessor::updateLatency()
{
	...
	foreach(LatencyAdder**, it, latencies)
		totalLatency += (*it)->getLatency(realtime);

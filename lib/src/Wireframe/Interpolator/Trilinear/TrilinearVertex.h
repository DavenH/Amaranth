...
public:
    ...
    void addOwner(TrilinearCube* cube);

    void removeOwner(TrilinearCube* cube)            { owners.removeFirstMatchingValue(cube); }
    void setMaxSharpness()                      { values[Curve] = 1.f;          }
    bool isOwnedBy(TrilinearCube* cube) const        { return owners.contains(cube); }

...
float EnvelopeMesh::getPositionOfCubeAt(TrilinearCube* line, MorphPosition pos) {
    if(line == nullptr) {
        return -1.f;
    }

    pos.time = 0;
    TrilinearCube::ReductionData reduce;
...
void EnvelopeMesh::clampSharpness(TrilinearCube* line) {
    if (line->getCompPath() >= 0) {
        return;
    }

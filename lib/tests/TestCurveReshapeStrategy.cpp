...
TEST_CASE("Curve reshape follows the gesture direction toward its selected control",
...
TEST_CASE("Curve reshape keeps sharpening after crossing its selected control",
        "[interaction][curve][reshape]") {
...
TEST_CASE("Curve reshape does not compensate an axis whose paired vertex moves",
        "[interaction][curve][reshape]") {
    REQUIRE(CurveReshapeStrategy::hiddenDimensionScale(true, 0.5f, 0.f, 1.f) == 1.f);
    REQUIRE(CurveReshapeStrategy::hiddenDimensionScale(false, 0.5f, 0.f, 1.f) == 2.f);
    REQUIRE(CurveReshapeStrategy::hiddenDimensionScale(false, 0.f, 0.f, 1.f) == 1.f);
}

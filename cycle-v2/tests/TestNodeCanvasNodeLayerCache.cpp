...
TEST_CASE("Node layer cache reuses only complete presentation keys",
    ...
    Rectangle<float> bounds { 10.f, 20.f, 180.f, 140.f };
    ...
    cache.beginFrame();
    const NodeCanvasNodeLayerCacheAccess first = access(3, 4, false);
    ...
    cache.beginFrame();
    REQUIRE(access(3, 4, false).hit);
    REQUIRE(cache.endFrame().hits == 1);

    bounds = bounds.translated(17.25f, -8.5f);
    cache.beginFrame();
    const NodeCanvasNodeLayerCacheAccess translated = access(3, 4, false);
    REQUIRE(translated.hit);
    Image translatedOutput(Image::ARGB, 240, 200, true);
    Graphics translatedGraphics(translatedOutput);
    cache.draw(translatedGraphics, translated);
    REQUIRE(translatedOutput.getPixelAt(
            roundToInt(bounds.getCentreX()),
            roundToInt(bounds.getCentreY())) == Colours::red);
    REQUIRE(cache.endFrame().hits == 1);
    ...
    cache.clear();
    cache.beginFrame();
    REQUIRE_FALSE(access(3, 4, false).hit);
    REQUIRE(cache.endFrame().misses == 1);
    ...
    cache.beginFrame();
    REQUIRE_FALSE(access(3, 4, true).hit);
    REQUIRE_FALSE(access(7, 4, true).hit);
    REQUIRE_FALSE(access(7, 8, true).hit);
    REQUIRE_FALSE(access(7, 8, true, 2.f).hit);
...
TEST_CASE("Node layer cache invalidates only the changed runtime preview",
    ...
    const auto access = [&](const Node& node, const NodePreviewResult& preview) {
        return cache.access(node, node.bounds, 2, 3, &preview, false, 1.f);
    };

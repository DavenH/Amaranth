...
5. The final visual review found the global collapse button ambiguous beside the Spy minimize button. Remove the expanded-state button and its normal keyboard target; keep the separate Guide and Spy minimize controls. Ignore saved global collapsed state so an older preference cannot hide both dock families without a visible way to restore them.
...
- Routed interaction tests/fixtures: click and wheel in a Spy tile versus adjacent transparent canvas; Guide vertical scroll and keyboard reveal; Spy horizontal scroll; Guide hover tether; selection and double-click editor launch; expanded editor close and dock focus; Guide/Spy minimize and Spy-row resize.
...
- Focused automation passes for vertical Guide overflow and reset, keyboard traversal, Guide and Spy drawers, tether routing, and canvas wheel routing through unused Spy-row space. The obsolete collapsed-dock fixture was removed after the global control was removed.
...
- A post-commit narrow-window review found that expanded editors could extend into the Guide column. `WorkspaceDock::editorAvailableBounds()` now defines the editor region left of Guides; native editors, the Guide editor, Spy detail, and automation inspection use it. Geometry checks at 1000×700 and 800×600 assert the expanded editor stays inside that region, and the focused canvas suite passes with 175 assertions.
- A user screenshot exposed scattered Spy controls when the graph had no probes: the label and empty tile sat at the left while refresh/minimize were at the right and the shared collapse button was centred. The Spy header now groups all four controls in a compact left-aligned row, and the empty preview tile is omitted. The focused canvas suite passes with 185 assertions including control-position checks; an empty-Spy fixture captures the resulting canvas, and the populated floating-Spy fixture still passes.
- A subsequent user review identified the down-chevron collapse button as ambiguous. The expanded dock no longer paints, hits, or focuses it, and saved collapsed state is ignored. Guide and Spy minimize controls remain available. Clicking canvas content after selecting a Guide now clears the Guide selection and its linked Trimesh highlight. The focused suite passes 182 assertions in 16 cases; the Guide click-off and keyboard fixtures pass, and a canvas capture confirms the button is absent.
